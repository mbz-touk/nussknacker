...
object LoggedUser {
  ...
  def apply(authenticatedUser: AuthenticatedUser, rules: List[ConfigRule], processCategories: List[String]): LoggedUser = {
    ...
    apply(id = authenticatedUser.id, username = authenticatedUser.username, authenticatedUser.roles, rulesSet = rulesSet)
  }
  ...
  def apply(id: String, username: String, roles: List[String], rulesSet: RulesSet): LoggedUser = {
    if (rulesSet.isAdmin) {
