...
trait AuthenticationResources extends Directives {
  ...
  final def routeWithPathPrefix(): Route = pathPrefix("authentication" / name.toLowerCase() ) { route() }

  protected def route(): Route = path("settings") {
    get { complete { frontendSettings } }
  }
}
